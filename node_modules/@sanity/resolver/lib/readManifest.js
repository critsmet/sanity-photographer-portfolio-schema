...
function handleManifestReadError(err, options) {
  if (err.code === 'ENOENT' && options.plugin) {
    ...
    const help = `See ${(0, _generateHelpUrl.default)('missing-plugin-sanity-json')}`;
...
function parseManifest(rawData, options) {
  ...
  const manifest = (0, _validateManifest.default)(parsedManifest);
...
function readManifest(opts = {}) {
  ...
  const manifestPath = _path.default.join(options.manifestDir || basePath, 'sanity.json');
