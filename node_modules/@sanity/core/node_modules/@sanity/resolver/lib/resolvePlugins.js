...
function resolvePlugin(options) {
  ...
  if (sync) {
    const manifestDir = isDirPlugin ? resolver(_path.default.resolve(parentDir, name)) : resolvePluginPath({
      name,
      basePath,
      parentPluginPath
    }, sync);
    const manifest = (0, _readManifest.default)({
      sync,
      basePath,
      manifestDir,
      plugin: pluginName
  ...
  const dirResolver = Promise.resolve(isDirPlugin ? resolver(_path.default.resolve(parentDir, name)) : resolvePluginPath({
    name,
    basePath,
    parentPluginPath
  }, sync));
...
function resolvePluginPath(plugin, sync) {
  ...
  let locations = [_path.default.join(plugin.basePath, 'plugins', pluginDir), _path.default.join(plugin.basePath, 'plugins', plugin.name), _path.default.join(plugin.basePath, 'node_modules', pluginDir)];
  ...
  if (sync) {
    const location = locations.find(_pathExists.default.sync);
...
function readPluginName(parent, dir) {
  const pkgPath = _path.default.join(parent, dir, 'package.json');

  const manifestPath = _path.default.join(parent, dir, 'sanity.json');
