...
const resolveProjectRoot = _resolveProjectRoot.default;
...
function resolveTreeSync(options) {
  ...
  const manifest = (0, _readManifest.default)(options);
...
function _resolveTreeAsync() {
  _resolveTreeAsync = _asyncToGenerator(function* (options) {
    const projectManifest = yield (0, _readManifest.default)(options);
    ...
    const flattened = withRoot.reduce(_flattenTree.default, withRoot);
    const deduped = (0, _removeDuplicatePlugins.default)(flattened);
    return deduped;
  });
  return _resolveTreeAsync.apply(this, arguments);
}
...
function resolveTree(opts = {}) {
  ...
  if (options.resolveProjectRoot) {
    // @todo Sync vs async
    const resolveOpts = Object.assign({}, options, {
      sync: true
    });
...
function mergeResult(plugins, options = {}) {
  ...
  const result = {
    definitions,
    implementations,
    plugins // Find plugins that define parts, and do a basic validation on the syntax
  ...
  const partPlugins = plugins.map(plugin => {
    ...
    if (!Array.isArray(plugin.manifest.parts)) {
      const help = `See ${(0, _generateHelpUrl.default)('plugin-parts-syntax')}`;
...
function assignPartImplementation(plugin, part, implementations, definitions, options) {
  ...
  if (prevDefinition && !prevDefinition.isAbstract) {
    const existing = `"${prevDefinition.plugin}" (${prevDefinition.path})`;
    const current = `"${plugin.name}" (${plugin.path})`;
    throw new Error(`${`Plugin ${current} tried to implement part "${partName}", which is already declared` + ` as a non-overridable part by ${existing} - ` + 'See '}${(0, _generateHelpUrl.default)('implement-non-overridable-part')}`);
...
function getImplementationDeclaration(plugin, part, options) {
  ...
  const isLib = options.useCompiledPaths || plugin.path.split(_path.default.sep).indexOf('node_modules') !== -1;
  const isDotPath = /^\.{1,2}[\\/]/.test(part.path);
  const basePath = isDotPath ? plugin.path : _path.default.join(plugin.path, (isLib ? paths.compiled : paths.source) || '');
  const filePath = _path.default.isAbsolute(part.path) ? part.path : _path.default.resolve(_path.default.join(basePath, part.path));
  return {
    plugin: plugin.name,
    path: filePath
  };
