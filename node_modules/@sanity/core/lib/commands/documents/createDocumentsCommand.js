...
var _default = {
  ...
  action: function () {
    var _action = _asyncToGenerator(function* (args, context) {
      ...
      const _args$extOptions = args.extOptions,
      ...
      if (file) {
        ...
        const content = json5.parse((yield fse.readFile(contentPath, 'utf8')));
        const result = yield writeDocuments(content, operation, client);
      ...
      const defaultValue = id && (yield client.getDocument(id)) || {
        _id: docId,
        _type: 'specify-me'
      };
      ...
      function _readAndPerformCreatesFromFile() {
        _readAndPerformCreatesFromFile = _asyncToGenerator(function* (filePath) {
          ...
          try {
            const writeResult = yield writeDocuments(content, operation, client);
...
function getResultMessage(result, operation) {
  ...
  const _result$results$reduc = result.results.reduce((acc, res) => {
