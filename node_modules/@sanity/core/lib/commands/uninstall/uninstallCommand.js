...
function _removeConfiguration() {
  _removeConfiguration = _asyncToGenerator(function* (workDir, fullName, shortName, prompt) {
    ...
    try {
      const localChecksum = yield (0, _generateConfigChecksum.default)(localConfigPath);
      const sameChecksum = yield (0, _pluginChecksumManifest.hasSameChecksum)(workDir, fullName, localChecksum);

      const _ref = yield promptOnAlteredConfiguration(shortName, sameChecksum, prompt),
            deleteConfig = _ref.deleteConfig;
...
function _removeFromSanityManifest() {
  _removeFromSanityManifest = _asyncToGenerator(function* (workDir, pluginName) {
    const manifest = yield (0, _readLocalManifest.default)(workDir, 'sanity.json');
