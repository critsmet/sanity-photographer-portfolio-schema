...
var _default = {
  ...
  action: function () {
    var _action = _asyncToGenerator(function* (args, context) {
      ...
      const _ref = yield Promise.all([client.datasets.list().then(sets => sets.map(ds => ds.name)), client.request({
        uri: '/features'
      })]),
            _ref2 = _slicedToArray(_ref, 2),
            datasets = _ref2[0],
            projectFeatures = _ref2[1];

      if (flags.visibility && !allowedModes.includes(flags.visibility)) {
        throw new Error(`Visibility mode "${flags.visibility}" not allowed`);
      }

      const datasetName = yield dataset || (0, _datasetNamePrompt.default)(prompt);
