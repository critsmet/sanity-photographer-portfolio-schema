...
module.exports = schemaDef => {
  const validated = (0, _validateSchema.default)(schemaDef.types).getTypes();
  const validation = (0, _groupProblems.default)(validated);
  const hasErrors = validation.some(group => group.problems.some(isError));
  let types = [];

  if (!hasErrors) {
    types = [...schemaDef.types, _slug.default, _geopoint.default, _schema2.default, _imageAsset.default, _fileAsset.default, _imageCrop.default, _imageHotspot.default, _imageMetadata.default, _imageDimensions.default, _imagePalette.default, _imagePaletteSwatch.default].filter(Boolean);
  }
