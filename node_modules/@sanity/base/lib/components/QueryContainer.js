...
var _default = (0, _reactPropsStream.streamingComponent)(receivedProps$ => {
  const _createEventHandler = (0, _reactPropsStream.createEventHandler)(),
  ...
  const queryResults$ = queryProps$.pipe((0, _operators.switchMap)(queryProps => {
    const query$ = (0, _listenQuery.listenQuery)(queryProps.query, queryProps.params).pipe((0, _operators.map)(createResultChildProps), (0, _operators.share)());
    return (0, _rxjs.merge)((0, _rxjs.of)({
      loading: true
    }).pipe((0, _operators.delay)(400), (0, _operators.takeUntil)(query$)), query$);
  }));
  const childProps$ = queryResults$.pipe((0, _operators.startWith)(INITIAL_CHILD_PROPS), (0, _operators.catchError)((err, caught$) => (0, _rxjs.concat)((0, _rxjs.of)(createErrorChildProps(err)), onRetry$.pipe((0, _operators.take)(1), (0, _operators.mergeMapTo)(caught$)))), (0, _operators.scan)((prev, next) => _objectSpread({}, prev, next)));
  return (0, _rxjs.combineLatest)(receivedProps$, childProps$).pipe((0, _operators.map)(([receivedProps, queryResult]) => {
    const children = receivedProps.children,
