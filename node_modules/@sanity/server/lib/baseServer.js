...
const getDocumentComponent = basePath => (0, _resolver.resolveParts)({
  basePath
}).then(res => {
...
function getDocumentElement({
  ...
  const assetHashes = hashes || {}; // Project filesystem base path

  return getDocumentComponent(basePath).then(Document => _react.default.createElement(Document, Object.assign({
    // URL base path
...
function applyStaticRoutes(app, config = {}) {
  const staticPath = config.staticPath || _path.default.join(__dirname, '..', 'public');

  const staticBasePath = (0, _getStaticBasePath.default)(config);
  ...
  app.get('*', (req, res) => {
    ...
    return getDocumentElement(config).then(doc => res.send(`<!doctype html>${_server.default.renderToStaticMarkup(doc)}`)).catch(err => {
