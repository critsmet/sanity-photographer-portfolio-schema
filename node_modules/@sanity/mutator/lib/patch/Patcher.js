...
class Patcher {
  ...
  apply(value) {
    ...
    const accessor = new _ImmutableAccessor.default(value);
...
function process(matcher, accessor) {
  let result = accessor; // Every time we execute the matcher a new set of leads is generated. Each lead
  ...
  const _matcher$match = matcher.match(accessor),
