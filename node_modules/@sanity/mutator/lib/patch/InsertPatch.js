...
function minIndex(targets, accessor) {
  let result = (0, _min2.default)((0, _util.targetsToIndicies)(targets, accessor)); // Ranges may be zero-length and not turn up in indices

  targets.forEach(target => {
    if (target.isRange()) {
      const _target$expandRange = target.expandRange(),
...
function maxIndex(targets, accessor) {
  let result = (0, _max2.default)((0, _util.targetsToIndicies)(targets, accessor)); // Ranges may be zero-length and not turn up in indices

  targets.forEach(target => {
    if (target.isRange()) {
      const _target$expandRange2 = target.expandRange(),
