...
class Parser {
  ...
  consume() {
    const result = this.peek(); // console.log("consumed", result)
  ...
  probe(pattern) {
    const token = this.peek(); // console.log("Probing", token, "for", pattern)
...
function mergeUnions(union1, union2) {
  ...
  (0, _uniq2.default)(Object.keys(union1).concat(Object.keys(union2))).forEach(key => {
