...
class Document {
  ...
  considerIncoming() {
    ...
    let nextMut; // Filter mutations that are older than the document
  ...
  updateConsistencyFlag() {
    ...
    const isConsistent = this.pending.length == 0 && this.submitted.length == 0 && this.incoming.length == 0; // Update the consistency state, taking care not to update the timestamp if we were inconsistent and still are
  ...
  applyIncoming(mut) {
    ...
    if (this.anyUnresolvedMutations()) {
      ...
      if (_debug.default.enabled) {
        (0, _debug.default)(`Incoming mutation ${mut.transactionId} appeared while there were pending or submitted local mutations`);
        (0, _debug.default)(`Submitted txnIds: ${this.submitted.map(m => m.transactionId).join(', ')}`);
        (0, _debug.default)(`Pending txnIds: ${this.pending.map(m => m.transactionId).join(', ')}`);
  ...
  rebase() {
    ...
    const changed = !(0, _isEqual2.default)(this.EDGE, oldEdge);
