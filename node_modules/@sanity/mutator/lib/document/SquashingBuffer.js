...
class SquashingBuffer {
  ...
  constructor(doc) {
    ...
    this.BASIS = doc;
    this.PRESTAGE = doc;
    this.dmp = new _diffMatchPatch.default();
  }
  ...
  addOperation(op) {
    ...
    if (op.patch && op.patch.set && op.patch.id === this.PRESTAGE._id && Object.keys(op.patch).length == 2) {
      ...
      const unoptimizable = {}; // Apply all optimisable keys in the patch
  ...
  optimiseSetOperation(path, nextValue) {
    ...
    const matches = (0, _extractWithPath.default)(path, this.PRESTAGE); // If we are not overwriting exactly one key, this cannot be optimised, so we bail
    ...
    const match = matches[0]; // If the value of the match is an array or object, we cannot safely optimise this since the meaning
    ...
    const canonicalPath = (0, _arrayToJSONMatchPath.default)(match.path); // Store this operation, overwriting any previous operations touching this same path
  ...
  stashStagedOperations() {
    ...
    const nextOps = []; // Extract the existing outgoing operations if any
