...
function createBufferedDocument(documentId, serverEvents$, doCommit) {
  const bufferedDocument = (0, _createObservableBufferedDocument.createObservableBufferedDocument)(serverEvents$, doCommit);
...
function createDocumentStore({
  ...
  function checkoutPair(idPair) {
    ...
    const serverEvents$ = serverConnection.byIdPair({
      publishedId,
      draftId
    }).pipe((0, _operators.share)());
    const draft = createBufferedDocument(draftId, serverEvents$.pipe((0, _operators.filter)(isEventForDocId(draftId))), doCommit);
    const published = createBufferedDocument(publishedId, serverEvents$.pipe((0, _operators.filter)(isEventForDocId(publishedId))), doCommit);
    return {
      draft,
      published
    };
