...
const viewModeSettings = _settings.default.forKey('inspect-view-preferred-view-mode');

function mapReceivedPropsToChildProps(props$) {
  const onViewModeChange = nextViewMode => viewModeSettings.set(nextViewMode.id);

  const viewModeSetting$ = viewModeSettings.listen('parsed').pipe((0, _operators.map)(id => VIEW_MODES.find(mode => mode.id === id)));
  return (0, _rxjs.combineLatest)(props$, viewModeSetting$).pipe((0, _operators.map)(([props, viewMode]) => _objectSpread({}, props, {
    viewMode,
    onViewModeChange
  })));
}
