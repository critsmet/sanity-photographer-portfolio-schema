...
function resolvePanes(structure, ids) {
  const waitStructure = (0, _isSubscribable.default)(structure) ? (0, _rxjs.from)(structure) : (0, _rxjs.of)(structure);
  return waitStructure.pipe((0, _operators.switchMap)(struct => resolveForStructure(struct, ids)));
}
...
function resolveForStructure(structure, ids) {
  return _rxjs.Observable.create(subscriber => {
    ...
    const subscriptions = []; // Start with all-loading state
    ...
    function subscribeForUpdates(pane, index, context, resolverArgs) {
      const source = (0, _serializeStructure.default)(pane, context, resolverArgs);
