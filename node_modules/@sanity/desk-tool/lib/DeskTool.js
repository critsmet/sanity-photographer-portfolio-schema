...
const hasLoading = panes => panes.some(item => item === _resolvePanes.LOADING);
...
const loadStructure = () => {
  ...
  try {
    const mod = require('part:@sanity/desk-tool/structure?') || _defaultStructure.default;
  ...
  if (!isStructure(structure)) {
    return (0, _rxjs.throwError)(new Error(`Structure needs to export a function, an observable, a promise or a stucture builder, got ${typeof structure}`));
...
const maybeSerialize = structure => structure && typeof structure.serialize === 'function' ? structure.serialize({
  path: []
}) : structure;
...
_temp = _class = class DeskTool extends _react.default.Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    if (!(0, _shallowEquals.default)(nextRouterState.panes, prevRouterState.panes) || nextRouterState.editDocumentId !== prevRouterState.editDocumentId || nextRouterState.legacyEditDocumentId !== prevRouterState.legacyEditDocumentId || nextRouterState.type !== prevRouterState.type || nextRouterState.action !== prevRouterState.action) {
    ...
    return !(0, _shallowEquals.default)(nextState, this.state);
  }

  maybeHandleOldUrl() {
    const navigate = this.props.router.navigate;
    const _this$props$router$st = this.props.router.state,
  ...
  getFallbackKeys() {
    ...
    return routerState.type && routerState.editDocumentId ? [`${routerState.type}-${routerState.editDocumentId}`] : EMPTY_PANE_KEYS;
  }
