...
function withPatchSubscriber(ComposedComponent) {
  ...
  return _temp = _class = class SubscribePatch extends _react.default.Component {
    constructor(...args) {
      ...
      _defineProperty(this, "subscribe", subscriber => {
        return this.context.formBuilder.onPatch(({
          snapshot,
          patches
        }) => {
          const selfPath = this.context.getValuePath();
          const filtered = patches.filter(patch => startsWith(patch.path, selfPath)).map(patch => _objectSpread({}, patch, {
  ...
  }, _defineProperty(_class, "displayName", `withPatches(${ComposedComponent.displayName || ComposedComponent.name})`), _defineProperty(_class, "contextTypes", {
