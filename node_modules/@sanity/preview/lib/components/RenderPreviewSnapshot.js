...
function resolvePreview(type) {
  const fromPreview = (0, _get2.default)(type, 'preview.component');

  if (fromPreview) {
    return fromPreview;
  }

  const custom = _previewResolver.default && (0, _previewResolver.default)(type);
...
function RenderPreviewSnapshot(props) {
  ...
  const PreviewComponent = resolvePreview(type); // TODO: Bjoerge: Check for image type with "is()"

  const renderAsBlockImage = layout === 'block' && type && type.name === 'image';

  const preview = _react.default.createElement(PreviewComponent // Render media always until we have schema functionality for determining if there is media
  , _extends({
    media: () => undefined
  }, rest, {
    value: snapshot,
